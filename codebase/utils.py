...
def sample_gaussian(m, v):
    ...
    for i in range(num):
        # mi = m[i, :]
        # vi = v[i, :]
        # zi = np.random.multivariate_normal(mi, np.diag(vi), 1)

        zi = torch.distributions.multivariate_normal.MultivariateNormal(m[i, :], torch.diag(v[i, :])).sample()
        # zi = mm.sample()

        zi = torch.reshape(zi, (1, dim))
...
def gaussian_parameters(h, dim=-1):
    ...
    # m, h = torch.split(h, h.size(-1) // 2, dim=dim)
    v = F.softplus(h) + 1e-8
    return m, v
